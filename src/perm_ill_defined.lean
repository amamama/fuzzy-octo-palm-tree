/-
abbrev ℕ := Nat
def bije {α β} (f: α → β) := (∀y: β, ∃x: α, f x = y) ∧ (∀x x', f x = f x' → x = x')
def bije_contraposition {α β} {f: α → β} (h: bije f) : (∀y, ∃x, f x = y) ∧ (∀x x', x ≠ x' → f x ≠ f x') := by {
  simp [bije] at h;
  have ⟨hl, hr⟩ := h;
  apply And.intro;
  assumption;
  intro x x' h1 h2;
  specialize hr x x' h2;
  contradiction;
}
...
syntax "bije" : tactic
macro_rules
  | `(tactic| bije) => `(tactic| simp [Bijection, bije]; apply And.intro; )

def Bijection.comp {α β γ} (g: Bijection β γ) (f: Bijection α β) : Bijection α γ := ⟨λx => g.val (f.val x), by {
  bije <;>
  have ⟨gl, gr⟩ := g.property <;>
  have ⟨fl, fr⟩ := f.property;
  case left => {
    intro y;
    specialize gl y;
    have ⟨glw, glp⟩ := gl;
...
def Bijection.inv {α β} (f: Bijection α β) : Bijection β α := ⟨λy => let ⟨fl, fr⟩ := f.property; let ⟨w, _⟩ := fl y; w, by {
  simp [bije];
  intro x;
